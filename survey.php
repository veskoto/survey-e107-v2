...
function show_survey($snum)
{
	...
	{  
     ...
     $JobAvail = true;
		//  testing parameters in URL  
		if($row['survey_neededpar']) {		 	
		  $tmp = explode(".", e_QUERY);   // $tmp, because $qs is used
		  $par_01 = varset($tmp[1],0);	 	// Usually a numeric parameter, or numeric news item number, but don't presume yet
			$par_02 = varset($tmp[2],0);    // Usually a numeric parameter, or numeric news item number, but don't presume yet
 
			$where = 'id ="'.$par_01.'"';
	 		if($sql->retrieve('jobs_wpjb_job', 'id', $where)) {  /*everything ok*/}
			else {
			    $JobAvail = false;
					/*    
					$ERROR_MESSAGE = $template[$surveytemplate]['error'];    
					$where = 'message_id =' . intval($row['survey_error1']);
					if ($parerror = $sql->retrieve('survey_messages', 'message_text', $where ))
					{ 
  
						$var = array(
							'ERROR_MESSAGE' => $tp->parseTemplate($tp->toHTML($parerror, false, 'TITLE')),
						);
						$ret.= $tp->simpleParse($ERROR_MESSAGE, $var);
...
//		extract($row);
		...
		if ($row['survey_message1'] && $JobAvail)
		{  
		...
		else
		{
      	...
      	$ret .= "<tr><td colspan='2'>{$survey_message}</td></tr>";
			...
			{        
				...
				if(!$field['field_hidden'] && $field['field_type'] != 9){    // not calculation
		      	...
		      	$ret .= "\n<tr>"; 
					...
					if($field['field_type'] != 13) //not inline radio  not full line
					{
							if($field['field_type'] != 6) //separator
							{
								if (deftrue('BOOTSTRAP'))
								{
						     	$ret .= "<div class='col-md-3 text-left'>";
									$ret .= show_form_question($field);
									$ret .= "</div>";
									$ret .= "<div class='col-md-8 text-left'>";
								}
								else
								{
...
if($_POST['submit']){
  ...
  $mailtext .= "URL: ".e_SELF."\n";  
  $mailtext .= "Query: ".e_QUERY."\n";  
  $mailtext .= "Prvé zobrazenie: ".$_POST['survey_time']."\n";  
  $trvanie = time() - $_POST['survey_time']   ; 
  $mailtext .= "Trvanie: ".$trvanie."\n";  
  // put this in email only if it's set to use
  if($row['survey_neededpar']) {
    $tmp = explode(".", e_QUERY);   // $tmp, because $qs is used
		$par_01x = varset($tmp[1],'chyba 1');	 	// Usually a numeric parameter, or numeric news item number, but don't presume yet
		$par_02x = varset($tmp[2],'chyba 2');    // Usually a numeric parameter, or numeric news item number, but don't presume yet
		
     
	  $mailtext .= "Par1: [".$par_01x."]\n";
	  $mailtext .= "Par2: [".$par_02x."]\n\n";
  }
...
//	header('Refresh: 4; URL=http://www.lugera.sk/');
	require_once(FOOTERF);
	exit;
}
